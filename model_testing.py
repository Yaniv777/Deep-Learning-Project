...
def model_predict(model_path):
    ...
    open_dir = pathlib.Path(r"C:\Users\Student\OneDrive\Documents\Deep-Learning-Project-main\‏‏eyesDataset\Test\open_look")
    # open eyes test directory path
    open_eyes_paths = list(open_dir.glob('*.jpeg'))  # list of the images in open_dir

    closed_dir = pathlib.Path(r"C:\Users\Student\OneDrive\Documents\Deep-Learning-Project-main\‏‏eyesDataset\Test\closed_look")
    # closed eyes test directory path
    eyes_paths = os.listdir(closed_dir)  # list of the images in closed_dir
    
    for i in range(len(open_eyes_paths)):
        tempath = os.path.join(open_dir, open_eyes_paths[i])
        eyes_paths.append(tempath)
        eyes_paths[i] = os.path.join(closed_dir, eyes_paths[i])
...
def model_score(model_path):
    ...
    model_path = pathlib.Path(model_path)
    model = tf.keras.models.load_model(model_path)
    test_dir = pathlib.Path(r"C:\Users\Student\OneDrive\Documents\Deep-Learning-Project-main\‏‏eyesDataset\Test")  # test dataset path
    open_dir = os.path.join(test_dir, r'open_look')
    closed_dir = os.path.join(test_dir, r'closed_look')
    print (len(os.listdir(open_dir)))
    if not len(os.listdir(open_dir)) == len(os.listdir(closed_dir)):
        margin = abs(len(os.listdir(open_dir)) - len(os.listdir(closed_dir)))
        if len(os.listdir(open_dir)) > len(os.listdir(closed_dir)):
            deleting_images.remove_margins(open_dir, margin)
        else:
            deleting_images.remove_margins(closed_dir, margin)
    test_ds = tf.keras.utils.image_dataset_from_directory(
        ...
        batch_size=len(os.listdir(open_dir)),
