...
def image_selection():
    images_dir = pathlib.Path(r"D:\Yaniv\Downloads")
    root.filename = filedialog.askopenfilename(initialdir=images_dir, title="Select An Image To Test")
    if root.filename.endswith('.jpg') or root.filename.endswith('.jpeg') or root.filename.endswith('.png'):
        global img
        img = root.filename
        success_label = Label(root, text="Great Image!", fg="green").pack()
        my_img = ImageTk.PhotoImage(Image.open(img))
        img_label = Label(image=my_img)
        img_label.pack()
        model_testing.model_working(eye_model)
        messagebox.showinfo("Eye Status", "")
    else:
        failed_label = Label(root, text="Enter An Image!", fg="red").pack()

            
def image_selection_button():
    image = Button(root, text="Select Image", command=image_selection)
    image.pack()
